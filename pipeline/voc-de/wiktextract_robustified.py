...
# from wiktionary.
#
# Robustified for Crash-Resume-Skip operations with Checkpointing.
#
...
import sys
import glob
import resource  # Critical for fixing Lua stack overflows
import re
import tarfile
import tempfile
import time
...
# Global variable to hold the context in worker processes
worker_wxr: Optional[WiktextractContext] = None

# --- CHECKPOINT CONFIGURATION ---
STATUS_DIR = "extraction_status"          # Temporary crash detection files
BLACKLIST_FILE = "skipped_crashed_pages.txt" # Pages that caused crashes (PERMANENT SKIP)
DONE_FILE = "completed_pages.log"         # Pages successfully processed (CHECKPOINT)


def page_handler(
    page: Page,
) -> tuple[str, list[dict[str, str]], CollatedErrorReturnData]:
    # --- CRASH DETECTION START ---
    pid = os.getpid()
    status_file = os.path.join(STATUS_DIR, f"worker_{pid}.processing")
    
    try:
        with open(status_file, "w", encoding="utf-8") as f:
            f.write(page.title)
            # CRITICAL ADDITION: Force write to disk immediately. 
            # Without this, a hard segfault might kill the process before the buffer flushes.
            f.flush()
            os.fsync(f.fileno())
    except Exception:
        pass
    ...
    with tempfile.TemporaryDirectory(prefix="wiktextract") as tmpdirname:
        ...
        try:
            ...
            # Cleanup status file on success
            ...
            return page.title, page_data, worker_wxr.wtp.to_return()
            
        except BaseException as e:
            # Handle KeyboardInterrupt cleanly so we don't leave zombie status files
            ...
            # Even on Python Exception (soft crash), remove status file
            if os.path.exists(status_file):
                try:
                    os.remove(status_file)
                except OSError:
            ...
            return page.title, [], worker_wxr.wtp.to_return()
...
def parse_wiktionary(
    ...
    """Parses Wiktionary from the dump file."""
...
def check_error(
    ...
    if prefix:
        msg = prefix + ": " + msg
    # print(msg) # Suppressed noisy output
    config = wxr.config
...
def init_worker(wxr: WiktextractContext) -> None:
    ...
    CRITICAL: Sets stack limit to infinity to prevent Lua Panics.
    """
    global worker_wxr

    # --- SAFETY FIX: UNLIMITED STACK ---
    # This fixes the "lua panic: not enough memory" error.
    try:
        resource.setrlimit(
        ...
        # Fallback for systems that don't allow INFINITY (e.g., macOS/Containers)
        try:
...
def detect_previous_crashes() -> set[str]:
    """
    Scans the STATUS_DIR for files left over from a previous run.
    These represent pages that caused a hard crash (segfault/panic).
    """
    crashed_titles = set()

    if not os.path.exists(STATUS_DIR):
        return crashed_titles

    files = glob.glob(os.path.join(STATUS_DIR, "*.processing"))
    if not files:
        return crashed_titles

    logger.warning(f"⚠️ Found {len(files)} interrupted pages from previous run. Marking as crashed.")
    ...
    for fpath in files:
        try:
            with open(fpath, "r", encoding="utf-8") as f:
                title = f.read().strip()
                if title:
                    crashed_titles.add(title)
            # Cleanup
            os.remove(fpath)
        except Exception as e:
            logger.error(f"Error reading crash file {fpath}: {e}")

    return crashed_titles


def get_safe_page_iterator(
    wxr: WiktextractContext,
    process_ns_ids: list[int],
    search_pattern: str | None,
    blacklist: set[str]
):
    """
    Yields pages but skips those in the blacklist (crached or finished).
    """
    # Get iterator from WTP
    iterator = wxr.wtp.get_all_pages(
        process_ns_ids, True, "wikitext", search_pattern
    )

    for page in iterator:
        if page.title in blacklist:
            # Optional: Don't log "done" pages to avoid spamming console
            # logger.info(f"🚫 Skipping: {page.title}")
            continue
        yield page
...
def reprocess_wiktionary(
    ...
    # 2. Load Checkpoints (Crashes + Completed)
    blacklist = set()
    
    # Load crashes (Permanent Skip)
    if os.path.exists(BLACKLIST_FILE):
        with open(BLACKLIST_FILE, "r", encoding="utf-8") as f:
            for line in f:
                blacklist.add(line.strip())
                
    # Load completed pages (Checkpoint Skip)
    completed_count = 0
        ...
        with open(DONE_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if title:
                    blacklist.add(title)
                    completed_count += 1
        logger.info(f"✅ Resuming: Skipping {completed_count} already completed pages.")

    # 3. Detect New Crashes from last run (Files left on disk)
    new_crashes = detect_previous_crashes()
    if new_crashes:
        blacklist.update(new_crashes)
        with open(BLACKLIST_FILE, "a", encoding="utf-8") as f:
            for title in new_crashes:
                f.write(f"{title}\n")
        logger.warning(f"Added {len(new_crashes)} pages to crash blacklist.")
    ...
    done_file_handle = open(DONE_FILE, "a", encoding="utf-8")
        ...
        while True:
            try:
                with ProcessPoolExecutor(
                    ...
                    safe_iter = get_safe_page_iterator(
                        wxr, process_ns_ids, search_pattern, blacklist
                    )
                    ...
                    for processed_pages, (page_title, page_data, wtp_stats) in enumerate(results):
                        ...
                        # --- CHECKPOINT SAVE ---
                        done_file_handle.write(f"{page_title}\n")
                        done_file_handle.flush()
                        blacklist.add(page_title)
                ...
                # Exit loop if no crashes
                break

            except BrokenProcessPool:
                restart_count += 1
                logger.error(f"CRITICAL: Worker Crashed (Segfault/OOM). Restart attempt {restart_count}/{max_restarts}")
                ...
                crashed_titles = detect_previous_crashes()
                
                if not crashed_titles:
                    logger.error("Could not identify causing page. Trying generic restart.")
                
                blacklist.update(crashed_titles)
                with open(BLACKLIST_FILE, "a", encoding="utf-8") as f:
                    for title in crashed_titles:
                        f.write(f"{title}\n")
                
                logger.info(f"Skipping {len(crashed_titles)} crashed page(s) and restarting pool...")
                wxr.remove_unpicklable_objects()
                continue
                ...
                logger.error(f"Unexpected error in reprocess loop: {e}\n{format_exc()}")
                out_f.flush()
                break
    ...
    # Cleanup status dir
    try:
        for f in glob.glob(os.path.join(STATUS_DIR, "*")):
            os.remove(f)
        os.rmdir(STATUS_DIR)
    except:
        pass
