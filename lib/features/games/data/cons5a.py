...
# 6. Filter: only keep words that appear in at least 2 sources
print("Filtering words that appear in at least 2 sources...")
df_final['source_count'] = df_final['Source'].apply(lambda x: len(x.split(',')) if x else 0)
df_final = df_final[df_final['source_count'] >= 2].copy()
df_final = df_final.drop('source_count', axis=1)

print(f"Words remaining after filtering: {len(df_final)}")
