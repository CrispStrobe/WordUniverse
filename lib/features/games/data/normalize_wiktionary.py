...
def create_full_normalized_db(dataset, db_path):
    ...
    print("\n### Creating FULL normalized schema (with all fields) ###")
    ...
    cursor.execute("CREATE TABLE derived_terms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, derived_word TEXT NOT NULL, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE related_terms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, related_word TEXT NOT NULL, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    ...
    cursor.execute("CREATE TABLE other_pos (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, pos TEXT, word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    ...
    cursor.execute("CREATE TABLE hypernyms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, hypernym_word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE hyponyms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, hyponym_word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE holonyms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, holonym_word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE meronyms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, meronym_word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE coordinate_terms (id INTEGER PRIMARY KEY, entry_id INTEGER NOT NULL, coordinate_word TEXT, FOREIGN KEY (entry_id) REFERENCES entries(id))")
    cursor.execute("CREATE TABLE expressions (id INTEGER PRIMARY KEY, sense_id INTEGER NOT NULL, expression TEXT, roman TEXT, gloss TEXT, FOREIGN KEY (sense_id) REFERENCES senses(id))")
    cursor.execute("CREATE TABLE proverbs (id INTEGER PRIMARY KEY, sense_id INTEGER NOT NULL, proverb TEXT, roman TEXT, gloss TEXT, FOREIGN KEY (sense_id) REFERENCES senses(id))")

    conn.commit()
    
    print("\n### Populating database (this will take a while) ###")
    ...
    def populate_simple_relation(entry_id, data_list, table_name, column_name):
        for item in data_list:
            word_text = item.get('word') if isinstance(item, dict) else item
            if word_text:
                cursor.execute(f'INSERT INTO {table_name} (entry_id, {column_name}) VALUES (?, ?)', (entry_id, word_text))

    def populate_sense_relation(sense_id, data_list, table_name):
        for item in data_list:
            if isinstance(item, dict):
                cursor.execute(f'INSERT INTO {table_name} (sense_id, {table_name[:-1]}, roman, gloss) VALUES (?, ?, ?, ?)', 
                             (sense_id, item.get(table_name[:-1]), item.get('roman'), item.get('gloss')))
    ...
    for idx, entry in enumerate(tqdm(dataset, desc="Processing entries"), 1):
        ...
        if entry.get('senses'):
            for sense_data in entry['senses']:
                ...
                if sense_data.get('expressions'):
                    db_stats['expressions'] += len(sense_data['expressions'])
                    populate_sense_relation(sense_id, sense_data['expressions'], 'expressions')
                
                if sense_data.get('proverbs'):
                    db_stats['proverbs'] += len(sense_data['proverbs'])
                    populate_sense_relation(sense_id, sense_data['proverbs'], 'proverbs')
        ...
        if entry.get('derived'):
            ...
            populate_simple_relation(entry_id, entry['derived'], 'derived_terms', 'derived_word')

        if entry.get('related'):
            db_stats['related_terms'] += len(entry['related'])
            populate_simple_relation(entry_id, entry['related'], 'related_terms', 'related_word')
        ...
        if entry.get('other_pos'):
            ...
            for op in entry['other_pos']:
                cursor.execute('INSERT INTO other_pos (entry_id, pos, word) VALUES (?, ?, ?)', (entry_id, op.get('pos'), op.get('word')))
        ...
        if entry.get('descendants'):
            ...
            for desc in entry['descendants']:
                cursor.execute('INSERT INTO descendants (entry_id, lang, word, roman) VALUES (?, ?, ?, ?)', (entry_id, desc.get('lang'), desc.get('word'), desc.get('roman')))

        if entry.get('hypernyms'):
            db_stats['hypernyms'] += len(entry['hypernyms'])
            populate_simple_relation(entry_id, entry['hypernyms'], 'hypernyms', 'hypernym_word')
            
        if entry.get('hyponyms'):
            db_stats['hyponyms'] += len(entry['hyponyms'])
            populate_simple_relation(entry_id, entry['hyponyms'], 'hyponyms', 'hyponym_word')

        if entry.get('holonyms'):
            db_stats['holonyms'] += len(entry['holonyms'])
            populate_simple_relation(entry_id, entry['holonyms'], 'holonyms', 'holonym_word')

        if entry.get('meronyms'):
            db_stats['meronyms'] += len(entry['meronyms'])
            populate_simple_relation(entry_id, entry['meronyms'], 'meronyms', 'meronym_word')
            
        if entry.get('coordinate_terms'):
            db_stats['coordinate_terms'] += len(entry['coordinate_terms'])
            populate_simple_relation(entry_id, entry['coordinate_terms'], 'coordinate_terms', 'coordinate_word')
    ...
    cursor.execute('CREATE INDEX idx_expressions_sense ON expressions(sense_id)')
    cursor.execute('CREATE INDEX idx_proverbs_sense ON proverbs(sense_id)')
    
    conn.commit()
    
    print("\n### Final Statistics (Full DB) ###")
...
- **other_pos**: (New Table) Alternative part-of-speech entries for this word.
...
def main():
    ...
    if args.skip_download:
        ...
        # We need to manually count lines for tqdm
        print("Counting lines in local file...")
        with open(jsonl_path, 'r', encoding='utf-8') as f:
            num_lines = sum(1 for line in f)
        dataset = (json.loads(line) for line in open(jsonl_path, 'r', encoding='utf-8'))
        dataset = tqdm(dataset, total=num_lines, desc="Processing local file")
    else:
        print(f"\n### Loading dataset: {args.source_repo_id} ###")
        dataset = load_dataset(args.source_repo_id, split='train')
        print(f"Loaded {len(dataset):,} entries")

    # --- 2. Normalize Database ---
    start_time = time.time()
    db_stats, db_filename = create_full_normalized_db(dataset, args.db_filename)
    end_time = time.time()
