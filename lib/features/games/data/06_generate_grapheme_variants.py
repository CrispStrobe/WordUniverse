...
OUTPUT_JSON_FILE = 'grundwortschatz_with_grapheme_variations.json'
...
FALLBACK_MAPPINGS = {
    ...
    'ɹ': [('r', 100.0)],  # English approximant r (in loanwords like "Crew", "Street")
    ...
    'o': [('o', 99.0), ('oh', 1.0)],  # close-mid back rounded (short o)
    
    # I/U-sounds (reduced vowels)
    '1': [('e', 70.0), ('i', 30.0)],  # close central unrounded (ɨ) - reduced vowel in "aneinander"
    'ɨ': [('e', 70.0), ('i', 30.0)],  # alternative notation for same sound
    
    # English vowels (in loanwords)
    'ʌ': [('u', 60.0), ('a', 40.0)],  # open-mid back unrounded (English "cup" sound in "Junkie")
    ...
    # Diphthong variant
    'ɔø': [('eu', 81.7), ('äu', 18.3)],  # espeak variant of ɔʏ (map to same as eu/äu)
    ...
    # English 'th' sounds (in English loanwords)
    'ð': [('th', 60.0), ('s', 30.0), ('d', 10.0)],  # voiced dental fricative (the, this, with)
    'θ': [('th', 60.0), ('s', 30.0), ('t', 10.0)],  # voiceless dental fricative (think, with)
...
def load_grapheme_mapping(grapheme_file):
    ...
    test_phonemes = ['ç', 'x', 'ʃ', 'ŋ', 'ɑ', 'ɾ', 'r', 'ɹ', 'ɡ', 'ɜ', 'ø', 'e', 'i', 'o', 'ʌ', '1', 'w', 'ʒ', 'ɔʏ', 'ɔø', 'ð', 'θ']
...
def enrich_with_variants(input_file, output_file, phoneme_to_graphemes):
    ...
    if 'metadata' in data:
        data['metadata']['hasGraphematicVariants'] = True
        data['metadata']['variantGenerationDate'] = None
    ...
    print(f"Words without variants: {words_skipped}")
