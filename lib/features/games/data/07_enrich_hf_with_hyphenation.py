...
# ============================================================================
# SCRIPT CONFIG (V23 - Hyphenation Reprocessing)
# ============================================================================
...
OUTPUT_JSON = 'grundwortschatz_safe_enriched_v23.json'
CHECKPOINT_FILE = 'enrichment_checkpoint_v23.json'
...
def load_checkpoint(
    ...
    reprocess_hyphenation: bool = False  ### NEW/MODIFIED ###
) -> Dict[str, Any]:
    """
    Load checkpoint data and sync it with any data already 
    ...
    for word_obj in vocabulary:
        if 'apiEnrichment' in word_obj:
            ...
            # ### NEW/MODIFIED LOGIC FOR REPROCESSING ###
            if status == 'no_data':
                ...
                if reprocess_hyphenation:
                    # NEW LOGIC: Only count as "done" if it has the hyphenation field
                    if 'hyphenation' in word_obj.get('apiEnrichment', {}):
                        words_from_data.add(word_id)
                    # If 'hyphenation' is not in the dict, we *don't* add it.
                    # This will force the main loop to re-process it.
                else:
                    # OLD LOGIC: Count all 'success' as done
                    words_from_data.add(word_id)
...
def build_enrichment_data(api_response: Dict[str, Any], primary_pos: str) -> Dict[str, Any]:
    """
    Build enrichment from the V22 API response.
    ...
    # 5. PRIORITIZE: Pronunciation, Examples, AND HYPHENATION (Wiktionary only)
    ...
    # ### NEW/MODIFIED - ADD HYPHENATION ###
    hyphenation = primary_entry.get('wiktionary_metadata', {}).get('hyphenation', [])
    if hyphenation:
        enrichment['hyphenation'] = hyphenation
    # ### END OF MODIFICATION ###

    # 6. AGGREGATE: Synonyms & Antonyms (Wiktionary + OdeNet)
    odenet_senses = semantics.get('odenet_senses', [])
    
    # --- MODIFIED TO HANDLE DICTIONARIES ---
    wikt_synonyms = semantics.get('wiktionary_synonyms', [])
    # Extract the 'synonym_word' string from each dictionary
    syn_set = set(
    ...
    # Extract the 'antonym_word' string from each dictionary
    ant_set = set(
    ...
    translations = semantics.get('wiktionary_translations', [])
    if translations:
        enrichment['wiktionary_translations'] = translations

    derived_terms = semantics.get('wiktionary_derived_terms', [])
    if derived_terms:
        enrichment['wiktionary_derived_terms'] = derived_terms

    related_terms = semantics.get('wiktionary_related_terms', [])
    if related_terms:
        enrichment['wiktionary_related_terms'] = related_terms
...
def enrich_vocabulary(
    ...
    """
    Main enrichment function (V23 UPDATE)
    
    - 'api_url' is now a parameter.
    - Removed client-side fallback logic.
    - Added 'reprocess_hyphenation' flag.
    """
    print("="*70)
    print("GRUNDWORTSCHATZ ENRICHMENT SCRIPT (V23 APP)")
    print("="*70)
    ...
    checkpoint = load_checkpoint(vocabulary, no_resume_errors, reprocess_hyphenation) ### NEW/MODIFIED ###
    ...
    try:
        for idx, word_obj in enumerate(tqdm(vocabulary, desc="Enriching")):
            ...
            try:
                ...
                # The build function is now updated for hyphenation
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enrich a Grundwortschatz JSON file using the CONSOLIDATED V23 Gradio API."
    )
    ...
    parser.add_argument(
        ...
        help=f"URL of the running V22/V23 Gradio App (default: {GRADIO_API_URL})"
    )
    ...
    # ### NEW/MODIFIED - ADD REPROCESS FLAG ###
    parser.add_argument(
    ...
    enrich_vocabulary(
        ...
        args.reprocess_hyphenation  ### NEW/MODIFIED ###
    )
