...
def enrich_data(input_file='voc_de.csv', output_file='voc_de_enriched.csv'):
    ...
    Preserves frequency data from the input file.
    """
    ...
    # Define morphological columns we want to add
    morph_cols = ['Case_spacy', 'Number_spacy', 'Degree_spacy', 'PronType_spacy', 'VerbForm_spacy']
    
    # Define frequency columns that should be preserved
    freq_cols = ['BUCHMEIER_rank', 'BUCHMEIER_freq', 'LEEDS_rank', 'LEEDS_freq', 
    ...
    # Combine for the final, ordered list: base + morph + frequency
    final_cols = base_cols + morph_cols + freq_cols

    # Add any missing columns from the *input* file for robustness
    for col in final_cols:
        if col not in df.columns:
            if col in morph_cols:
                # New morphological columns - will be filled by spaCy
                df[col] = ''
            elif col in freq_cols:
                # Frequency columns should exist, but add as empty if missing
                print(f"Warning: Frequency column '{col}' not found. Adding it as empty.")
                df[col] = ''
            else:
                # Other base columns
    ...
    for (index, row), doc in tqdm(zip(df.iterrows(), docs), total=len(df)):
        ...
        if token:
            ...
            # Use the helper function for all morph features
        ...
        # Set final morph vars. Since user has no source, we just use spaCy's
        ...
        enriched_row = {
            ...
            # Add all the morphological fields
        ...
        # Add frequency data from the input row
        for freq_col in freq_cols:
            enriched_row[freq_col] = row[freq_col]
    ...
    print("\n--- Sample with frequency data: ---")
    # Show a sample that includes frequency columns
    sample_cols = ['Word', 'Wortart', 'Genus', 'BUCHMEIER_rank', 'LEIPZIG_rank', 'HERMIT_rank']
    available_sample_cols = [col for col in sample_cols if col in df_enriched.columns]
    print(df_enriched[available_sample_cols].head(20).to_string())
