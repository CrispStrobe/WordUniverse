...
class GameProvider extends ChangeNotifier {
  ...
  final Map<String, SkillCategory> gameSkillMap = {
    ...
    'grossstadt_game': SkillCategories.getById(
        'basic_spelling')!, // Capitalization is spelling rules
    'grossschreib_game': SkillCategories.getById('basic_spelling')!,
    'verbtrenner_game':
        SkillCategories.getById('word_types')!, // Separable verbs
  ...
  void _loadSettingsFromPrefs() {
    _tasksCustomizationEnabled =
        _prefs.getBool('tasksCustomizationEnabled') ?? false;
    _taskWordLengthMin = _prefs.getDouble('taskWordLengthMin') ?? 2.0;
    _taskWordLengthMax = _prefs.getDouble('taskWordLengthMax') ?? 10.0;
    _taskIncludedSources =
        Set<String>.from(_prefs.getStringList('taskIncludedSources') ?? []);
    ...
    _activeVocabularySetIds =
        Set<String>.from(_prefs.getStringList('activeVocabularySetIds') ?? []);
    _selectedFontFamily =
        _prefs.getString('selectedFontFamily') ?? AppFonts.standard;
    ...
    final difficultyModeIndex =
        _prefs.getInt('difficultyMode') ?? DifficultyMode.normal.index;
    _difficultyMode = DifficultyMode
        .values[difficultyModeIndex.clamp(0, DifficultyMode.values.length - 1)];
    _useCustomProblemSettings =
        _prefs.getBool('useCustomProblemSettings') ?? false;
    _customOperations = Set<String>.from(
        _prefs.getStringList('customOperations') ??
  ...
  Future<void> _saveProgress() async {
    ...
    await _prefs.setInt('sessionsPlayed', _sessionsPlayed);
    ...
    await _prefs.setStringList('achievements',
  ...
  void advanceLevel(String gameType) {
    if (kDebugMode)
      debugPrint(
          '[GAME_PROVIDER] 📈 $gameType advancing to level ${(_gameProgress[gameType] ?? 1) + 1}');
  ...
  bool _checkSpellingMastery() {
    ...
    if (kDebugMode)
      debugPrint(
          '[GAME_PROVIDER] Spelling mastery @ Grade $_grade: ${stat.mastered}/${stat.tracked} (${stat.successRate * 100}%) ${hasMastery ? "✓" : "✗"}');
    return hasMastery;
  }
  ...
  void _checkAchievements() {
    ...
    if (_score >= 100 && !hasAchievement('first_century')) {
      newAchievements.add(Achievement(id: 'first_century'));
    }
    if (_score >= 500 && !hasAchievement('score_master')) {
      newAchievements.add(Achievement(id: 'score_master'));
    }
    ...
    if ((_gameProgress['word_sort_game'] ?? 0) >= 3 &&
        !hasAchievement('bubble_popper')) {
      newAchievements.add(Achievement(id: 'bubble_popper'));
    }
    if ((_gameProgress['word_find_game'] ?? 0) >= 3 &&
        !hasAchievement('puzzle_solver')) {
      newAchievements.add(Achievement(id: 'puzzle_solver'));
    }
    if ((_gameProgress['word_builder_game'] ?? 0) >= 3 &&
        !hasAchievement('number_walls_pro')) {
      newAchievements.add(Achievement(id: 'number_walls_pro'));
    }
    if ((_gameProgress['space_word_rescue'] ?? 0) >= 3 &&
        !hasAchievement('codebreaker_pro')) {
      newAchievements.add(Achievement(id: 'codebreaker_pro'));
    }
    if ((_gameProgress['wortbaumeister_game'] ?? 0) >= 3 &&
        !hasAchievement('master_builder')) {
      newAchievements.add(Achievement(id: 'master_builder'));
    }
    if ((_gameProgress['grossstadt_game'] ?? 0) >= 3 &&
        !hasAchievement('city_planner')) {
      newAchievements.add(Achievement(id: 'city_planner'));
    }
    if ((_gameProgress['grossschreib_game'] ?? 0) >= 3 &&
        !hasAchievement('connection_expert')) {
      newAchievements.add(Achievement(id: 'connection_expert'));
    }

    if ((_gameProgress['antonym_flash'] ?? 0) >= 3 &&
        !hasAchievement('antonym_ace')) {
      newAchievements.add(Achievement(id: 'antonym_ace'));
    }
    if ((_gameProgress['synonym_flash'] ?? 0) >= 3 &&
        !hasAchievement('synonym_scholar')) {
      newAchievements.add(Achievement(id: 'synonym_scholar'));
    }
    if ((_gameProgress['cloze_flash'] ?? 0) >= 3 &&
        !hasAchievement('cloze_master')) {
      newAchievements.add(Achievement(id: 'cloze_master'));
    }
    if ((_gameProgress['translation_flash'] ?? 0) >= 3 &&
        !hasAchievement('translation_titan')) {
      newAchievements.add(Achievement(id: 'translation_titan'));
    }
    if ((_gameProgress['reverse_translation_flash'] ?? 0) >= 3 &&
        !hasAchievement('reverse_linguist')) {
      newAchievements.add(Achievement(id: 'reverse_linguist'));
    }
    if ((_gameProgress['syllable_count'] ?? 0) >= 3 &&
        !hasAchievement('syllable_counter')) {
      newAchievements.add(Achievement(id: 'syllable_counter'));
    }
    if ((_gameProgress['expression_flash'] ?? 0) >= 3 &&
        !hasAchievement('expression_expert')) {
      newAchievements.add(Achievement(id: 'expression_expert'));
    }
    if ((_gameProgress['hypernym_flash'] ?? 0) >= 3 &&
        !hasAchievement('hypernym_hunter')) {
      newAchievements.add(Achievement(id: 'hypernym_hunter'));
    }
    if ((_gameProgress['word_class_flash'] ?? 0) >= 3 &&
        !hasAchievement('word_class_whiz')) {
      newAchievements.add(Achievement(id: 'word_class_whiz'));
    }
    if ((_gameProgress['proverb_cloze'] ?? 0) >= 3 &&
        !hasAchievement('proverb_sage')) {
      newAchievements.add(Achievement(id: 'proverb_sage'));
    }
    if ((_gameProgress['conjugation_drill'] ?? 0) >= 3 &&
        !hasAchievement('conjugation_king')) {
      newAchievements.add(Achievement(id: 'conjugation_king'));
    }
    if ((_gameProgress['verbtrenner_game'] ?? 0) >= 3 &&
        !hasAchievement('verb_splitter')) {
      newAchievements.add(Achievement(id: 'verb_splitter'));
    }
    if ((_gameProgress['definition_quiz'] ?? 0) >= 3 &&
        !hasAchievement('definition_wizard')) {
      newAchievements.add(Achievement(id: 'definition_wizard'));
    }
    if ((_gameProgress['sentence_completion'] ?? 0) >= 3 &&
        !hasAchievement('sentence_smith')) {
      newAchievements.add(Achievement(id: 'sentence_smith'));
    }
    if ((_gameProgress['spelling_spotter'] ?? 0) >= 3 &&
        !hasAchievement('spelling_sleuth')) {
      newAchievements.add(Achievement(id: 'spelling_sleuth'));
    }
    if ((_gameProgress['homophone_drill'] ?? 0) >= 3 &&
        !hasAchievement('homophone_hero')) {
      newAchievements.add(Achievement(id: 'homophone_hero'));
    }
    if ((_gameProgress['confusable_drill'] ?? 0) >= 3 &&
        !hasAchievement('confusable_pro')) {
      newAchievements.add(Achievement(id: 'confusable_pro'));
    }
    if ((_gameProgress['sri_review'] ?? 0) >= 5 &&
        !hasAchievement('review_regular')) {
      newAchievements.add(Achievement(id: 'review_regular'));
    }
    ...
    final gamesCompleted =
        _gameProgress.values.where((level) => level >= 1).length;
  ...
  double get averageLevel {
    ...
    final totalLevels =
        _gameProgress.values.fold(0, (sum, level) => sum + level);
    return totalLevels / _gameProgress.length;
  }

  // Grade progression
  bool canProgressToNextGrade() {
    final totalLevelsInGrade =
        _gameProgress.values.fold(0, (sum, level) => sum + level);
