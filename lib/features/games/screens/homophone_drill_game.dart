// lib/features/games/screens/homophone_drill_game.dart
//
// Homophone Drill (#38) + Confusable Drill (#40) — EN-only.
// A sentence with a blanked word is shown; the player picks the correct
// spelling. Mode selects homophones (sound-alikes) or confusables
// (semantic look-alikes like affect/effect, loose/lose).
...
class HomophoneDrillGame extends StatefulWidget {
  ...
  final HomophoneGameMode mode;
  const HomophoneDrillGame({
    super.key,
    required this.gradeLevel,
    this.mode = HomophoneGameMode.homophones,
  });

  @override
  State<HomophoneDrillGame> createState() => _HomophoneDrillGameState();
}

enum _Feedback { none, correct, incorrect }

class _HomophoneDrillGameState extends State<HomophoneDrillGame>
  ...
  List<HomophoneChallenge> _challenges = [];
  int _index = 0;
  int _correct = 0;
  int _total = 0;
  int? _selectedOption;
  ...
  void _handleTap(int optionIndex) {
    ...
    _sriService.recordResponse(
      ...
      baseWord: challenge.correctWord,
  ...
  void _showGameOver() {
    ...
    _gameProvider.reportOutcome(GameOutcome(
      gameType: widget.mode == HomophoneGameMode.confusables
          ? 'confusable_drill'
          : 'homophone_drill',
  ...
  Widget _buildSentenceCard(HomophoneChallenge challenge) {
  ...
  Widget _buildSentenceText(String sentence) {
    // Render "___" as a highlighted blank inside the sentence text.
  ...
  Widget _buildOptions(HomophoneChallenge challenge) {
    return Wrap(
      spacing: 10,
      runSpacing: 10,
  ...
  Widget _buildOption(HomophoneChallenge challenge, int index) {
  ...
  Widget _buildMeaningHints(HomophoneChallenge challenge) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(challenge.groupWords.length, (i) {
            final isCorrect =
                challenge.groupWords[i].toLowerCase() ==
                    challenge.correctWord.toLowerCase();
            return Padding(
