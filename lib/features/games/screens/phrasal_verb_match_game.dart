// lib/features/games/screens/phrasal_verb_match_game.dart
//
// Phrasal Verb Match (#47) — EN-only.
// A phrasal verb is shown (with an optional context sentence); the player
// picks its meaning from up to four choices. Distractor meanings are real
// meanings of other phrasal verbs.
//
// Data: the `phrasal_verbs` table in the EN database, loaded via
// VocabularyService.getPhrasalVerbs().
...
class PhrasalVerbMatchGame extends StatefulWidget {
  final GradeLevel gradeLevel;
  const PhrasalVerbMatchGame({super.key, required this.gradeLevel});

  @override
  State<PhrasalVerbMatchGame> createState() => _PhrasalVerbMatchGameState();
}

enum _Feedback { none, correct, incorrect }

class _PhrasalVerbMatchGameState extends State<PhrasalVerbMatchGame>
  ...
  List<PhrasalMatchChallenge> _challenges = [];
  int _index = 0;
  int _correct = 0;
  int _total = 0;
  int? _selectedOption;
  ...
  Future<void> _init() async {
    ...
    if (!_onboardingScheduled) {
      ...
      OnboardingOverlay.maybeShow(
        context,
        gameKey: 'phrasal_verb_match',
        title: s.phrasalVerbMatchTitle,
        steps: [
          OnboardingStep(icon: Icons.link, body: s.phrasalVerbMatchOnboard1),
          OnboardingStep(
              icon: Icons.checklist, body: s.phrasalVerbMatchOnboard2),
          OnboardingStep(
              icon: Icons.tips_and_updates, body: s.phrasalVerbMatchOnboard3),
        ],
      );
    }
  }
  ...
  Future<void> _buildChallenges() async {
    ...
    final built = buildPhrasalMatchChallenges(
      verbs: verbs,
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Padding(
        ...
        child: Text(
          S.of(context)!.phrasalVerbMatchEmpty,
  ...
  Widget _buildPrompt(PhrasalMatchChallenge challenge) {
    return AnimatedBuilder(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Text(
              S.of(context)!.phrasalVerbMatchPrompt,
  ...
  Widget _buildOption(PhrasalMatchChallenge challenge, int index) {
    ...
    return Padding(
      ...
      child: AnimatedBuilder(
        ...
        builder: (_, child) => Transform.scale(
          scale:
              hasAnswered && isCorrect ? 1.0 + (_pulseCtrl.value * 0.02) : 1.0,
          child: child,
        ),
