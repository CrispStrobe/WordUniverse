// lib/features/games/screens/antonym_flash_game.dart
//
// Antonym Flash — timed warm-up game.
// A word is shown; the player taps its antonym from 4 options within the
// session timer. DE uses OdeNet antonyms; EN uses WordNet (via apiEnrichment).
...
class AntonymFlashGame extends StatefulWidget {
  ...
  const AntonymFlashGame({super.key, required this.gradeLevel});
...
class _AntonymFlashGameState extends State<AntonymFlashGame>
  ...
  // Auto-advance delay after a tap
  ...
  void _buildChallenges() {
    final allWords = _vocabService
        ...
        .where((w) =>
            !w.isProperNoun &&
            (w.apiEnrichment?.antonyms.isNotEmpty ?? false) &&
            !w.word.contains('_') &&
            !w.word.contains(' '))
        .toList();
